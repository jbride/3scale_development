...
+
TIP: To check if your shell still has these environment variables set, run  `echo $OCP_WILDCARD_DOMAIN` command. If the variables are no longer set, return to the first lab in this course and follow the steps there to set them again.
+
...
In the previous module, a custom module for Lua was written using Red Hat 3scale API Management to include SOAP headers and add basic routing of a request to a SOAP API endpoint. The reference below shows one example of how this was done.

.Reference
* link:http://middlewareblog.redhat.com/2017/08/30/integrating-soap-based-web-services-into-red-hat-3scale-api-management/["Integrating SOAP-Based Web Services into Red Hat 3scale API Management^"]
