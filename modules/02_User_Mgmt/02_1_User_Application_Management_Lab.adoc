...
In 3scale API Management, multiple users can be created for the same account. When the developer account for RHBank was created in the previous lab, the account was created for a specific email ID, username, and password. The Organization/Group Name provided is the name of the account. The default user created is an `admin` user of the developer account, and as such, can invite other users to the account.


=== Send Invitation to User's Email Address
...
NOTE: Start this procedure after completing the lab *2.1 Development Portal Customization*.
