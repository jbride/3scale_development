...
. Navigate to *Developers* and delete the default `Developer` user (if it exists) :
+
image::images/3scale_amp_admin_delete_developers.png[]
+
NOTE:  Deletion of this `Developer` user will reduce possible confusion later on in this lab.
If this default `Developer` account is not deleted, new API services that are defined will automatically have a default _application_ (with corresponding API keys) created for this user.
