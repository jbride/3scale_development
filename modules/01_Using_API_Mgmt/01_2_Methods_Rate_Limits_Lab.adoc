...
+
NOTE: If your Business service is running in the same OpenShift cluster as your APICast gateway, then you can use service endpoint instead of the Route endpoint. The service endpoint will be the service host and port information provided in the Service description, as shown below. 
+
...
Once you have rate limits in place, these limits will control the responses a developer receives when they make authorization request calls to the backend service using 3scale. The limits are configured in the Admin Portal, and will be enforced by the APICast gateway during service invocation. The gateway receives the configuration information from the 3scale backend which contains the rate limits for the different application plans within each service. 
...
. *authrep* response from 3scale backend updates local apicast cache.
. APIcast will reject all subsequent inbound requests if backend determines that rate limit is exceeded.
